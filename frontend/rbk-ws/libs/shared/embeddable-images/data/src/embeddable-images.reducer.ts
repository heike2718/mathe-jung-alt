...
import {
  ...
  initialEmbeddableImageResponseDto,
...
export const embeddableImagesFeature = createFeature({
  ...
  reducer: createReducer<EmbeddableImagesState>(
    ...
    on(embeddableImagesActions.iMAGE_INFO_SELECTED, (state, action) => ({
      ...state,
      selectedEmbeddableImageInfo: action.embeddableImageInfo,
    })),
    ...
    on(embeddableImagesActions.rESET_STATE, () => initialState)
  ),
});
