...
import { authActions, fromAuth } from '@rbk-ws/core/data';
import { AuthResult, Benutzerart, isAdmin, User } from '@rbk-ws/core/model';
...
export class AuthFacade {
  ...
  readonly user$: Observable<User> = this.#store.select(fromAuth.userFull).pipe(
    filterDefined,
    switchMap(user =>
      of({ fullName: user.fullName, isAdmin: isAdmin(user), anonym: user.anonym, benutzerart: user.benutzerart })
    )
  );

  readonly userIsLoggedIn$: Observable<boolean> = this.#store
    .select(fromAuth.userFull)
    .pipe(switchMap(user => of(!user.anonym)));

  readonly userIsLoggedOut$: Observable<boolean> = this.userIsLoggedIn$.pipe(switchMap(li => of(!li)));
      ...
      if (authResult.state) {
        ...
        if (authResult.state === 'signup') {
          ...
          this.#messageService.info(
            'Ihr Benutzerkonto wurde angelegt und muss noch aktiviert werden. Bitte schauen Sie in Ihrem Postfach nach.'
          );
        }
      } else {
        window.location.hash = '';
