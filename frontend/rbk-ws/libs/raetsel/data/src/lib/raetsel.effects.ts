...
export class RaetselEffects {
  ...
  raetselSelected$ = createEffect(() => {
    return this.#actions.pipe(
      ...
      map(raetselDetails =>
        raetselActions.rAETSEL_DETAILS_LOADED({ raetselDetails: raetselDetails, navigateTo: 'raetsel/details' })
      )
    );
  });
  ...
  generateRaetselPng$ = createEffect(() => {
    return this.#actions.pipe(
      ...
      switchMap(action =>
        this.#raetselHttpService
          .generateRaetselPNGs(action.raetselID, action.font, action.schriftgroesse, action.layoutAntwortvorschlaege)
          .pipe(
  ...
  generateRaetselPDF$ = createEffect(() => {
    return this.#actions.pipe(
      ...
      switchMap(action =>
        this.#raetselHttpService
          .generateRaetselPDF(action.raetselID, action.font, action.schriftgroesse, action.layoutAntwortvorschlaege)
          .pipe(
