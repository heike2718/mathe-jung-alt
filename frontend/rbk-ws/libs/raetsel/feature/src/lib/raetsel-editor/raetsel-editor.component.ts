...
export class RaetselEditorComponent implements OnInit, OnDestroy {
  ...
  #warnungIncludegraphics =
    'Bitte den \\includegraphics-Befehl nicht manuell einfügen. Er wird beim Hochladen einer neuen Datei vom System generiert. Der generierte Pfad darf nicht geändert werden!';
  ...
  ngOnInit(): void {
    this.#combinedSubscription = combineLatest([
      this.raetselFacade.editRaetselPayload$,
      this.#coreFacade.alleDeskriptoren$,
      this.#authFacade.userIsRoot$,
    ]).subscribe(([guiEditRaetselPayload, alleDeskriptoren, root]) => {
      ...
      this.#selectedQuellenart = new GuiQuellenartenMap().getLabelOfQuellenart(
        this.guiEditRaetselPayload.editRaetselPayload.quelle.quellenart
      );
      ...
      this.SelectItemsComponentModel = this.raetselFacade.initSelectItemsComponentModel(
        this.guiEditRaetselPayload.editRaetselPayload.deskriptoren,
        alleDeskriptoren
      );
      this.embeddableImageInfosFrage = guiEditRaetselPayload.embeddableImageInfos.filter(
        info => info.textart === 'FRAGE'
      );
      this.embeddableImageInfosLoesung = guiEditRaetselPayload.embeddableImageInfos.filter(
        info => info.textart === 'LOESUNG'
      );
    ...
    this.#combinedEmbeddableImageVorschauSubscription = combineLatest([
    ...
    this.#medienForQuelleSubscription = combineLatest([
      this.raetselFacade.editRaetselPayload$,
      this.raetselFacade.medienForQuelle$,
    ]).subscribe(([payload, medien]) => {
      this.medienForQuelle = medien;
      if (payload.editRaetselPayload.quelle.mediumUuid) {
        const filtered: MediumQuelleDto[] = medien.filter(
          m => m.id === this.guiEditRaetselPayload.editRaetselPayload.quelle.mediumUuid
        );
  ...
  get avFormArray() {
    return this.form.controls['antwortvorschlaege'] as UntypedFormArray;
  }
  get antwortvorschlaegeFormGroup() {
    return this.avFormArray.controls as UntypedFormGroup[];
  }
  ...
  generierenDiabled(): boolean {
    const embeddableImageInfosFrageOhneFile: EmbeddableImageInfo[] = this.embeddableImageInfosFrage.filter(
      gi => !gi.existiert
    );
    const embeddableImageInfosLoesungOhneFile: EmbeddableImageInfo[] = this.embeddableImageInfosLoesung.filter(
      gi => !gi.existiert
    );
    return (
      !this.form.valid ||
      this.antwortvorschlaegeErrors() ||
      embeddableImageInfosFrageOhneFile.length > 0 ||
      embeddableImageInfosLoesungOhneFile.length > 0
    );
  }
  ...
  #initForm() {
    const theGuiHerkunftstyp: GuiHerkunfsttyp = new GuiHerkunftstypenMap().getGuiHerkunftstyp(
      this.guiEditRaetselPayload.editRaetselPayload.herkunftstyp
    );
    const theGuiQuellenart: GuiQuellenart = new GuiQuellenartenMap().getGuiQuellenart(
      this.guiEditRaetselPayload.editRaetselPayload.quelle.quellenart
    );

    this.form.patchValue({
      schluessel: this.guiEditRaetselPayload.editRaetselPayload.schluessel,
      name: this.guiEditRaetselPayload.editRaetselPayload.name,
      herkunftstyp: theGuiHerkunftstyp.label,
      freigegeben: this.guiEditRaetselPayload.editRaetselPayload.freigegeben,
      antwortvorschlaegeEingebettet: this.guiEditRaetselPayload.editRaetselPayload.antwortvorschlaegeEingebettet,
      ...
      person: this.guiEditRaetselPayload.editRaetselPayload.quelle.person
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.person
        : '',
      jahr: this.guiEditRaetselPayload.editRaetselPayload.quelle.jahr
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.jahr
        : '',
      ausgabe: this.guiEditRaetselPayload.editRaetselPayload.quelle.ausgabe
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.ausgabe
        : '',
      seite: this.guiEditRaetselPayload.editRaetselPayload.quelle.seite
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.seite
        : '',
      klasse: this.guiEditRaetselPayload.editRaetselPayload.quelle.klasse
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.klasse
        : '',
      stufe: this.guiEditRaetselPayload.editRaetselPayload.quelle.stufe
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.stufe
        : '',
      pfad: this.guiEditRaetselPayload.editRaetselPayload.quelle.pfad
        ? this.guiEditRaetselPayload.editRaetselPayload.quelle.pfad
        : '',
  ...
  onSelectItemsComponentModelChanged($event: SelectItemsComponentModel) {
    if ($event) {
      ...
      selectedItems.forEach(item =>
        this.#selectedDeskriptoren.push({
  ...
  #handleQuellenartChanged(): void {
    if (this.guiEditRaetselPayload.editRaetselPayload.quelle.mediumUuid) {
      const filtered: MediumQuelleDto[] = this.medienForQuelle.filter(
        m => m.id === this.guiEditRaetselPayload.editRaetselPayload.quelle.mediumUuid
      );
      if (filtered.length > 0) {
        this.selectedMedium = filtered[0];
  ...
  cancelEdit() {
    if (
      this.guiEditRaetselPayload.editRaetselPayload.id !== 'neu' &&
      this.guiEditRaetselPayload.editRaetselPayload.schluessel
    ) {
  ...
  #readFormValues(): void {
    ...
    const c_schluessel = formValue['schluessel']
      ? formValue['schluessel'].trim()
      : this.guiEditRaetselPayload.editRaetselPayload.schluessel;
  ...
  #latexChanged(): boolean {
    ...
    const result =
      frageActual !== this.#editRaetselPayloadCache.frage || loesungActual !== this.#editRaetselPayloadCache.loesung;
  ...
  #openPrintDialog(outputformat: OutputFormat): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.raetselFacade.generiereRaetselOutput(
          this.guiEditRaetselPayload.editRaetselPayload.id,
          outputformat,
          font,
          schriftgroesse,
          layout
  ...
  #openEmbeddableImageVorschauDialog(vorschau: EmbeddableImageVorschau): void {
    const hinweis =
      'Die Grafik kann ersetzt werden, indem sie in der Detailansicht unter Grafiken ausgewählt und anschließend eine neue hochgeladen wird ' +
