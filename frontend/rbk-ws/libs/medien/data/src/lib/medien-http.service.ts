...
import { PageDefinition, QUERY_PARAM_LIMIT, QUERY_PARAM_OFFSET } from '@rbk-ws/core/model';
import { LinkedRaetsel, MediensucheResult, MediensucheTrefferItem, MediumDto } from '@rbk-ws/medien/model';
...
export class MedienHttpService {
  ...
  #url = '/raetselbaukasten/api/medien';
  ...
  findMedien(suchstring: string, pageDefinition: PageDefinition): Observable<MediensucheResult> {
    ...
    let params = new HttpParams().set(QUERY_PARAM_LIMIT, pageDefinition.pageSize).set(QUERY_PARAM_OFFSET, offset);
