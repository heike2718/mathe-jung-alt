...
import { fromAufgabensammlungen, aufgabensammlungenActions } from '@rbk-ws/aufgabensammlungen/data';
import {
  ...
  initialAufgabensammlungDetails,
...
export class AufgabensammlungenFacade {
  ...
  suchparameter$: Observable<AufgabensammlungenSuchparameter> = this.#store.select(
    fromAufgabensammlungen.suchparameter
  );
  editorContent$: Observable<AufgabensammlungBasisdaten> = this.#store
    .select(fromAufgabensammlungen.aufgabensammlungBasisdaten)
    .pipe(filterDefined);
  aufgabensammlungDetails$: Observable<AufgabensammlungDetails> = this.#store
    .select(fromAufgabensammlungen.aufgabensammlungDetails)
    .pipe(filterDefined);
  aufgabensammlungBasisdaten$: Observable<AufgabensammlungBasisdaten> = this.#store
    .select(fromAufgabensammlungen.aufgabensammlungBasisdaten)
    .pipe(filterDefined);
  aufgabensammlungselemente$: Observable<Aufgabensammlungselement[]> = this.#store.select(
    fromAufgabensammlungen.aufgabensammlungselemente
  );
  selectedAufgabensammlungselement$: Observable<Aufgabensammlungselement> = this.#store
    .select(fromAufgabensammlungen.selectedAufgabensammlungselement)
    .pipe(filterDefined);
  selectedElementImages$: Observable<GeneratedImages | undefined> = this.#store.select(
    fromAufgabensammlungen.selectedElementImages
  );
  ...
  triggerSearch(
    ...
    this.#store.dispatch(
      aufgabensammlungenActions.fIND_AUFGABENSAMMLUNGEN({ aufgabensammlungenSuchparameter, pageDefinition })
    );
  }
  ...
  selectAufgabensammlungselement(element: Aufgabensammlungselement): void {
    this.#store.dispatch(
      aufgabensammlungenActions.sELECT_AUFGABENSAMMLUNGSELEMENT({ aufgabensammlungselement: element })
    );
  }

  generiereArbeitsblatt(
    aufgabensammlungID: string,
    font: FontName,
    schriftgroesse: Schriftgroesse,
    layoutAntwortvorschlaege: LaTeXLayoutAntwortvorschlaege
  ): void {
    this.#store.dispatch(
      aufgabensammlungenActions.gENERIERE_ARBEITSBLATT({
        aufgabensammlungID,
        font,
        schriftgroesse,
  ...
  generiereKnobelkartei(
    ...
    this.#store.dispatch(
      aufgabensammlungenActions.gENERIERE_KNOBELKARTEI({
        aufgabensammlungID,
        font,
        schriftgroesse,
        layoutAntwortvorschlaege,
  ...
  generiereVorschau(
    ...
    this.#store.dispatch(
      aufgabensammlungenActions.gENERIERE_VORSCHAU({
        aufgabensammlungID,
        font,
        schriftgroesse,
        layoutAntwortvorschlaege,
  ...
  generiereLaTeX(
    ...
    this.#store.dispatch(
      aufgabensammlungenActions.gENERIERE_LATEX({ aufgabensammlungID, font, schriftgroesse, layoutAntwortvorschlaege })
    );
  }
  ...
  cancelEdit(aufgabensammlung: AufgabensammlungBasisdaten): void {
    if (aufgabensammlung.id === 'neu') {
      ...
      this.#store.dispatch(
        aufgabensammlungenActions.sELECT_AUFGABENSAMMLUNG({ aufgabensammlungId: aufgabensammlung.id })
      );
    }
  }
