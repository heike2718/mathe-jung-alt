...
import de.egladil.raetselbaukasten.domain.auth.login.AuthproviderUrlService;
import de.egladil.raetselbaukasten.domain.auth.login.LoginLogoutService;
...
public class SessionResource {
    ...
    @PermitAll
    @Operation(operationId = "logoutDev", summary = "entfernt die Session (nur Dev)")
    @Parameters({ @Parameter(in = ParameterIn.PATH, name = "sessionId", description = "id der Session"), })
    @APIResponse(
            name = "GetLoginUrlOKResponse",
            responseCode = "200",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = MessagePayload.class)))
