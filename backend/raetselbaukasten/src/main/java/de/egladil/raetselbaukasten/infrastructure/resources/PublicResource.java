...
/**
 * PublicResource stellt Resourcen ohne Berechtigungseinschränkungen zur
 * Verfügung.
 */
@Path("api/public")
public class PublicResource {
    ...
    @Parameters({ @Parameter(in = ParameterIn.HEADER, name = "X-CLIENT-ID", description = "ClientId", required = false),
    ...
    @APIResponse(name = "BadRequest", description = "Input-Validierung ging schief.", responseCode = "400")
	...
	// @formatter:on
        ...
        MinikaenguruAufgabenKlassenstufeDto aufgaben = minikaenguruService
                .getAufgabenFreigegebenerWettbewerb(jahr, schwierigkeitsgrad);
    ...
    @Operation(
            ...
            description = "Nur freigegebene Wettbewerbe werden geliefert.")
    @Parameters({ @Parameter(in = ParameterIn.HEADER, name = "X-CLIENT-ID", description = "ClientId", required = false),
            @Parameter(in = ParameterIn.PATH, name = "jahr", description = "Jahr des Minikänguru-Wettbewerbs"),
            @Parameter(
                    in = ParameterIn.PATH,
    ...
    @APIResponse(
            ...
            responseCode = "200",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = Quiz.class)))
    @APIResponse(name = "NotFound", description = "Gibt es nicht", responseCode = "404")
    @APIResponse(
            name = "ServerError",
            description = "Serverfehler",
            responseCode = "500",
            content = @Content(schema = @Schema(implementation = MessagePayload.class)))
    public Response generateQuizMinikaenguru(
            @Pattern(
                    regexp = MjaRegexps.VALID_JAHR,
                    message = "jahr enthält ungültige Zeichen oder hat nicht die Länge 4")
            @PathParam(value = "jahr") final String jahr,
            @PathParam(value = "klasse") final Schwierigkeitsgrad schwierigkeitsgrad) {

        Optional<Quiz> optResult = quizService.generateQuiz(Referenztyp.MINIKAENGURU, jahr, schwierigkeitsgrad);
