...
import de.egladil.raetselbaukasten.domain.upload.EmbeddableImageUplodService;
...
public class EmbeddableImagesResource {
    ...
    public Response generatePreview(
            @NotBlank @Pattern(regexp = MjaRegexps.VAILD_RELATIVE_PATH_EPS, message = "pfad enthält ungültige Zeichen")
            @QueryParam(value = "pfad") final String relativerPfad) {
    ...
    @APIResponse(
            ...
            responseCode = "403",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = MessagePayload.class)))
    @APIResponse(name = "Not Found", description = "Das Rätsel existiert nicht", responseCode = "404")
    @APIResponse(
    ...
    @APIResponse(
            ...
            description = "Akteur ist nicht Besitzer des Rätsels und auch kein ADMIN",
            responseCode = "403")
    @APIResponse(name = "Not Found", description = "Das Rätsel oder die Datei exisistieren nicht", responseCode = "404")
    @APIResponse(
            name = "ServerException",
            description = "sonstige Fehler",
            responseCode = "500",
    ...
    public Response updateEmbeddableImage(@Valid final ReplaceEmbeddableImageRequestDto requestDto) {
        ...
        // return Response.serverError().entity(MessagePayload.error("absichtlicher
        // Error")).build();
    }
