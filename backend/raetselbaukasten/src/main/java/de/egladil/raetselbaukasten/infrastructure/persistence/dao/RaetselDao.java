...
import de.egladil.raetselbaukasten.domain.utils.SetOperationUtils;
import de.egladil.raetselbaukasten.infrastructure.persistence.entities.PersistentesAufgabensammlungRaetselsucheItemReadonly;
import de.egladil.raetselbaukasten.infrastructure.persistence.entities.PersistentesRaetsel;
import de.egladil.raetselbaukasten.infrastructure.persistence.entities.PersistentesRaetselHistorieItem;
...
public class RaetselDao {
     ...
     *
     * @param neuesHistorieItem PersistentesRaetselHistorieItem
     */
    ...
    /**
     * Zählt alle RAETSEL, bei denen der Suchstring im Volltextindex enthalten ist.
     * Dieser umfasst NAME, KOMMENTAR, FRAGE und LOESUNG. Bei mehreren Worten werden
     * alle je nach suchmodus mit AND oder mit OR verknüpft.
     *
     * @param suchstring
     * @param suchmodus  SuchmodusVolltext mündet in AND oder OR, wenn der
     *                   Suchstring aus mehr als einem Wort besteht.
     * @return List
     */
    @SuppressWarnings("unchecked")
    public long countRaetselVolltext(final String suchstring, final SuchmodusVolltext suchmodus,
            final boolean nurFreigegebene) {
    ...
    /**
     * Gibt alle RAETSEL innerhalb des Paginators zurück, bei denen der Suchstring
     * im Volltextindex enthalten ist. Dieser umfasst NAME, KOMMENTAR, FRAGE und
     * LOESUNG. Bei mehreren Worten werden alle je nach suchmodus mit AND oder mit
     * OR verknüpft.
     *
     * @param suchstring
     * @param suchmodus     SuchmodusVolltext mündet in AND oder OR, wenn der
     ...
     */
    @SuppressWarnings("unchecked")
    public List<PersistentesRaetsel> findRaetselVolltext(final String suchstring, final SuchmodusVolltext suchmodus,
            final int limit, final int offset, final SortDirection sortDirection, final boolean nurFreigegebene) {
        ...
        stmt += sortDirection == SortDirection.desc ? " ORDER BY SCHLUESSEL desc" : " ORDER BY SCHLUESSEL";
    ...
    /**
     * Zählt alle Raetsel, deren Deskriptoren die deskriptorenIDs als Teilmenge
     * enthalten oder nicht enthalten.
     *
     * @param deskriptorenIDs       String die IDs der Deskriptoren
     * @param suchmodusDeskriptoren SuchmodusDeskriptoren
     * @return List
     */
    @SuppressWarnings("unchecked")
    public long countWithDeskriptoren(final String deskriptorenIDs, final SuchmodusDeskriptoren suchmodusDeskriptoren,
            final boolean nurFreigegebene) {

        String wrappedDeskriptorenIds = new SetOperationUtils().prepareForDeskriptorenLikeSearch(deskriptorenIDs);

        LOGGER
                .debug("deskriptoren={}, suchmodusDeskriptoren={}, nurFreigegebene={}", wrappedDeskriptorenIds,
                        suchmodusDeskriptoren, nurFreigegebene);
    ...
    /**
     * Sucht alle Raetsel, deren Deskriptoren die deskriptorenIDs als Teilmenge
     * enthalten oder nicht enthalten.
     *
     * @param deskriptorenIDs       String die IDs der Deskriptoren
     * @param suchmodusDeskriptoren TODO
     * @param limit                 int Anzahl Treffer in page
     * @param offset                int Aufsetzpunkt für page
     * @param sortDirection         SortDirection asc/desc
     * @return List
     */
    public List<PersistentesRaetsel> findWithDeskriptoren(final String deskriptorenIDs,
            final SuchmodusDeskriptoren suchmodusDeskriptoren, final int limit, final int offset,
            final SortDirection sortDirection, final boolean nurFreigegebene) {

        String wrappedDeskriptoren = new SetOperationUtils().prepareForDeskriptorenLikeSearch(deskriptorenIDs);

        LOGGER
                .debug("deskriptoren={}, suchmodusDeskriptoren={}, nurFreigegebene={}", wrappedDeskriptoren,
                        suchmodusDeskriptoren, nurFreigegebene);

        String queryName = queryNameFilteredSearch(suchmodusDeskriptoren, sortDirection, nurFreigegebene);

        LOGGER
                .debug("deskriptoren={}, suchmodus={}, limit={}, offset={}, queryName={}", wrappedDeskriptoren,
                        suchmodusDeskriptoren, limit, offset, queryName);

        if (nurFreigegebene) {

            return entityManager
                    .createNamedQuery(queryName, PersistentesRaetsel.class)
    ...
    String queryNameFilteredSearch(final SuchmodusDeskriptoren suchmodusDeskriptoren, final SortDirection sortDirection,
            final boolean nurFreigegebene) {
    ...
    /**
     * Zählt alle RAETSEL, bei denen die Volltextsuche Treffer ergibt (Worte werden
     * entweder mit AND oder mit OR verknüpft) und mit Deskriptoren LIKE oder not
     * LIKE. Der Volltextindex umfasst NAME, KOMMENTAR, FRAGE und LOESUNG.
     *
     * @param suchfilter      Suchfilter
     * @param nurFreigegebene boolean
     * @return List
     */
    @SuppressWarnings("unchecked")
    public long countRaetselWithFilter(final Suchfilter suchfilter, final boolean nurFreigegebene) {

        String wrappedDeskriptorenIds = new SetOperationUtils()
                .prepareForDeskriptorenLikeSearch(suchfilter.getDeskriptorenIds());

        LOGGER
                .debug("suchstring={}, deskriptoren={}, suchmodusVolltext={}, suchmodusDeskriptoren={}",
                        suchfilter.getSuchstring(), wrappedDeskriptorenIds, suchfilter.getModusVolltext(),
                        suchfilter.getModusDeskriptoren());

        String[] worte = StringUtils.split(suchfilter.getSuchstring(), ' ');

        String where = this.getVolltextMatcher(worte.length, suchfilter.getModusVolltext()) + " AND "
        ...
        Query query = this
                .createQueryAndReplaceSuchparameter(stmt, worte, Long.class)
                .setParameter("deskriptoren", wrappedDeskriptorenIds);
    ...
    /**
     * Zählt alle RAETSEL, bei denen die Volltextsuche Treffer ergibt (mehrere Worte
     * werden entweder mit AND oder mit OR verknüpft) und mit Deskriptoren LIKE oder
     * not LIKE. Der Volltextindex umfasst NAME, KOMMENTAR, FRAGE und LOESUNG.
     *
     * @param suchfilter      Suchfilter
     * @param limit           int Anzahl Treffer in page
     * @param offset          int Aufsetzpunkt für page
     ...
     */
    @SuppressWarnings("unchecked")
    public List<PersistentesRaetsel> findRaetselWithFilter(final Suchfilter suchfilter, final int limit,
            final int offset, final SortDirection sortDirection, final boolean nurFreigegebene) {
        ...
        Query query = this
                .createQueryAndReplaceSuchparameter(stmt, worte, PersistentesRaetsel.class)
                .setParameter("deskriptoren", wrappedDeskriptorenIds);
    ...
    public long countRaetselWithStatus(final boolean freigegeben) {
        ...
        @SuppressWarnings("unchecked")
        List<Long> trefferliste = entityManager
                .createNativeQuery(stmt)
                .setParameter("freigegeben", freigegeben)
                .getResultList();
    ...
    public int getMaximumOfAllSchluessel() {
        ...
        @SuppressWarnings("unchecked")
        List<String> trefferliste = entityManager
                .createNativeQuery(stmt)
                .setParameter("schluessel", "99999")
                .getResultList();
    ...
    public PersistentesRaetsel findWithSchluessel(final String schluessel) {
        ...
        if (trefferliste.size() > 1) {
            ...
            throw new MjaRuntimeException(
                    "Da ist in den Daten was faul: mehr als ein Raetsel mit schluessel=" + schluessel);
        }
    ...
    /**
     * Selektiert alle Raetsel, deren SCHLUESSEL in der gegebenen Collection
     * enthalten ist.<br>
     * <br>
     * <strong>Achtung:</strong> Es wird mit IN gesucht. Es gibt ein DB-Limit für
     * die Länge der Liste. Das wird aktuell nicht berücksichtigt. Es dürfte
     * momentan auch so groß sein, dass es nicht durch irgendwelche
     * Aufgabensammlungen gerissen werden kann. Es gibt ein Issue diesbezüglich:
     * https://github.com/heike2718/mathe-jung-alt/issues/105
     *
     * @param schluessel List
     * @return List
     */
    public List<PersistentesRaetsel> findWithSchluesselListe(final List<String> schluessel) {

        return entityManager
                .createNamedQuery(PersistentesRaetsel.FIND_WITH_SCHLUESSEL_LIST, PersistentesRaetsel.class)
                .setParameter("schluessel", schluessel)
                .getResultList();
    }
    ...
    Query createQueryAndReplaceSuchparameter(final String stmt, final String[] worte,
     ...
     *
     * @param raetselId String die UUID des RAETSELS. Sortiert ist nach Name der
     *                  Aufgabensammlungen.
     * @return List
     */
    public List<PersistentesAufgabensammlungRaetselsucheItemReadonly> findAllAufgabensammlungenWithRaetsel(
