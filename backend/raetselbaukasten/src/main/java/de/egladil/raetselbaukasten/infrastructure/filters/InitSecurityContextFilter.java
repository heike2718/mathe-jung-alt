...
import de.egladil.raetselbaukasten.infrastructure.cdi.AuthenticationContextImpl;
...
public class InitSecurityContextFilter implements ContainerRequestFilter {
    ...
    private static List<String> OPEN_DATA_PATHS = Arrays
            .asList(new String[] { "/api/public", "/api/restricted", "/api/session/logout",
                    "/api/session/authurls/login", "/api/session/login", "/api/session/authurls/signup",
                    "/api/about" });
    ...
    public void filter(final ContainerRequestContext requestContext) throws IOException {
        ...
        LOGGER
                .debug("stage={}, mockSession={}, path={}, noSessionRequired={}", configService.getStage(),
        ...
        if (!ConfigService.STAGE_PROD.equals(configService.getStage()) && configService.isMockSession()) {
            ...
            LOGGER
                    .warn("Achtung: mock-Session!!! check properties 'stage' und 'mock.session' [stage={}, mockSession=",
        ...
        // throw new MjaAuthRuntimeException("Unerwarterer Fehler bei Request " + method
        // + " path=" + path);
        // }
    }
    ...
    private void initMockSecurityContext(final ContainerRequestContext requestContext) {
        ...
        AuthenticatedUser user = new AuthenticatedUser(mockBenutzerid)
                .withFullName(mockBenutzerFullName)
                .withIdReference("bla")
                .withRoles(new String[] { mockBenutzerart })
                .withBenutzerart(benutzerart);
