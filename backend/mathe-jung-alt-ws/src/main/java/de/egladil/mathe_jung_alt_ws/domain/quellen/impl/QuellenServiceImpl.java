...
public class QuellenServiceImpl implements QuellenService {
	...
	@Override
	public List<QuelleReadonly> sucheQuellen(final String suchstring) {
		...
		List<QuelleReadonly> result = trefferliste.stream().map(pq -> mapFromDB(pq)).collect(Collectors.toList());
